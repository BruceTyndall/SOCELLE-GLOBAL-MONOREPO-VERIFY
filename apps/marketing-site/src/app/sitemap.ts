...
async function fetchBrandSlugs(): Promise<{ slug: string; updatedAt: string }[]> {
    try {
        // Replace with actual Supabase fetch when marketing-site is fully wired
        // const { data } = await supabase.from('brands').select('slug, updated_at').eq('is_published', true);
        // return (data ?? []).map(b => ({ slug: b.slug, updatedAt: b.updated_at }));
        return [];
    } catch {
        return [];
    }
...
async function fetchJobSlugs(): Promise<{ slug: string; updatedAt: string }[]> {
    try {
        // const { data } = await supabase.from('job_postings').select('slug, posted_at').eq('active', true);
        // return (data ?? []).map(j => ({ slug: j.slug, updatedAt: j.posted_at }));
        return [];
    } catch {
        return [];
    }
...
async function fetchEventSlugs(): Promise<{ slug: string; updatedAt: string }[]> {
    try {
        // const { data } = await supabase.from('events').select('id, date').order('date', { ascending: true });
        // return (data ?? []).map(e => ({ slug: e.id, updatedAt: e.date }));
        return [];
    } catch {
        return [];
    }
