// ═══════════════════════════════════════════════════════════════════════════
// CLEO DESIGN SYSTEM — FLUTTER TRANSLATION
// Warm cocoa glassmorphism · Shimmer pill buttons · Editorial typography
// ═══════════════════════════════════════════════════════════════════════════
//
// This file translates the Cleo web design system into Flutter widgets
// and tokens. It is ADDITIVE to the existing SocelleColors/SocelleTheme.
// Use CleoTheme for new intelligence surfaces (Feed, Discover, Profile).
...
//
//   CleoColors.primaryCocoa
//   CleoText.heading(text)
//   CleoPillButton(label: 'Get Started', variant: PillVariant.primary, tone: PillTone.glass)
//   CleoCard(child: ...)
//   CleoGlassCard(child: ...)
...
class CleoText {
  ...
  // Cleo uses PPNeueMontreal — we substitute with a geometric sans that ships
  // with Flutter or can be bundled. For now, use the existing Inter with Cleo
  ...
  static const displayLarge = TextStyle(
    ...
    fontWeight: FontWeight.w300,  // Cleo uses light for headings
    color: CleoColors.primaryCocoa,
    height: 1.08,
    letterSpacing: -1.5,
  );
  ...
  static const label = TextStyle(
    fontFamily: _fontFamily,
    fontSize: 13.8,
    fontWeight: FontWeight.w500, // Cleo uses 450, closest in Flutter is w500
    color: CleoColors.primaryCocoa,
    letterSpacing: 0,
  );
  ...
  static const labelUppercase = TextStyle(
    ...
    color: CleoColors.neutralBeige,
    letterSpacing: 0,  // Cleo notably has no letter-spacing on uppercase
  );
  ...
  static Widget eyebrow(String text, {Color? color}) {
    return Text(
      text.toUpperCase(),
      style: labelUppercase.copyWith(color: color ?? CleoColors.neutralBeige),
    );
  }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// 5. PILL BUTTON SYSTEM (The Cleo Button Component)
// ─────────────────────────────────────────────────────────────────────────────
...
/// Cleo-style glassmorphic pill button.
///
/// ```dart
/// CleoPillButton(
///   label: 'Get the app',
...
/// )
/// ```
class CleoPillButton extends StatefulWidget {
  const CleoPillButton({
    super.key,
    required this.label,
  ...
  @override
  State<CleoPillButton> createState() => _CleoPillButtonState();
}

class _CleoPillButtonState extends State<CleoPillButton> {
  ...
  Color get _textColor {
    if (widget.isDisabled) {
      return _isLight
          ? CleoColors.primaryCocoa.withValues(alpha: 0.2)
          : Colors.white.withValues(alpha: 0.2);
    }
    return _isLight ? CleoColors.primaryCocoa : Colors.white;
  }

  // ── Background color by variant × tone × state ──
  Color get _backgroundColor {
    if (widget.isDisabled) return CleoColors.glassLight;

    if (_isPressed) {
      return switch ((widget.variant, widget.tone)) {
        (PillVariant.primary, PillTone.light) => CleoColors.primaryCocoa.withValues(alpha: 0.4),
        ...
        (PillVariant.primary, PillTone.glass) when !widget.isDarkTheme =>
            CleoColors.bgMain.withValues(alpha: 0.3),
        (PillVariant.primary, PillTone.glass) => Colors.black.withValues(alpha: 0.2),
        (PillVariant.secondary, PillTone.light) => CleoColors.primaryCocoa.withValues(alpha: 0.4),
    ...
    if (widget.isActive) {
      return switch ((widget.variant, widget.tone)) {
        (PillVariant.primary, PillTone.light) => CleoColors.bgMain.withValues(alpha: 0.4),
        (PillVariant.primary, PillTone.dark) => Colors.white.withValues(alpha: 0.3),
        (PillVariant.primary, PillTone.glass) when !widget.isDarkTheme =>
            CleoColors.bgMain.withValues(alpha: 0.4),
    ...
    return switch ((widget.variant, widget.tone)) {
      (PillVariant.primary, PillTone.light) => CleoColors.glassLight,
      (PillVariant.primary, PillTone.dark) => CleoColors.glassDark,
      (PillVariant.primary, PillTone.glass) when !widget.isDarkTheme => CleoColors.glassLight,
      (PillVariant.primary, PillTone.glass) => CleoColors.glassDarkBg,
      (PillVariant.secondary, PillTone.light) => CleoColors.bgMain.withValues(alpha: 0.1),
      (PillVariant.secondary, PillTone.dark) => CleoColors.glassLight,
  ...
  Border? get _border {
    return switch ((widget.variant, widget.tone)) {
      (PillVariant.secondary, PillTone.light) =>
          Border.all(color: CleoColors.bgMain.withValues(alpha: 0.8), width: 1),
  ...
  List<BoxShadow>? get _shadows {
    ...
    return switch ((widget.variant, widget.tone)) {
      (PillVariant.primary, _) => [CleoColors.shadowLight],
      (PillVariant.secondary, _) => [CleoColors.shadowMedium],
      _ => null,
    };
  }
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        duration: CleoAnimation.dSnap,
        curve: CleoAnimation.snap,
              ...
              child: Padding(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(
                      widget.label,
                      style: CleoText.buttonLabel.copyWith(
                        color: _textColor,
...
/// Paints the Cleo shimmer gradient border — a vertical gradient that
...
class CleoGlassCard extends StatelessWidget {
  const CleoGlassCard({
    super.key,
    required this.child,
    this.isDark = false,
  ...
  @override
  Widget build(BuildContext context) {
    final radius = borderRadius ?? CleoRadius.card(context);
    ...
    return ClipRRect(
      ...
      child: BackdropFilter(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: isDark
                ? CleoColors.glassDark
                : CleoColors.glassLight,
            borderRadius: radius,
            border: Border.all(
              color: isDark
  ...
  Widget build(BuildContext context) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        padding: padding,
        decoration: BoxDecoration(
          color: color ?? CleoColors.bgNearWhite,
          borderRadius: radius,
          border: Border.all(
            color: CleoColors.neutralBeige.withValues(alpha: 0.2),
            width: 1,
          ),
          boxShadow: [
            BoxShadow(
...
class CleoSectionLabel extends StatelessWidget {
  const CleoSectionLabel(this.text, {super.key, this.color});
  ...
  Widget build(BuildContext context) {
    return Text(
      text.toUpperCase(),
      style: CleoText.labelUppercase.copyWith(
        color: color ?? CleoColors.neutralBeige,
      ),
    );
  }
}
...
class CleoChip extends StatelessWidget {
  const CleoChip({
    super.key,
    required this.label,
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: isSelected
              ? (isDark ? Colors.white.withValues(alpha: 0.2) : CleoColors.primaryCocoa)
              : (isDark ? Colors.white.withValues(alpha: 0.05) : CleoColors.glassLight),
          ...
          border: Border.all(
            color: isSelected
                ...
                : (isDark
                    ...
                    : CleoColors.neutralBeige.withValues(alpha: 0.3)),
            width: 0.8,
          ),
        ),
        child: Text(
          label,
          style: CleoText.label.copyWith(
            fontSize: 14,
            color: isSelected
                ? (isDark ? Colors.white : CleoColors.bgMain)
                : (isDark ? Colors.white.withValues(alpha: 0.7) : CleoColors.primaryCocoa),
          ),
        ),
      ),
    );
...
// ─────────────────────────────────────────────────────────────────────────────
// 10. NAV PILL BAR (horizontal pill navigation — Cleo nav style)
// ─────────────────────────────────────────────────────────────────────────────

class CleoNavPill extends StatelessWidget {
  const CleoNavPill({
    super.key,
    required this.items,
                  ...
                  child: AnimatedContainer(
                    ...
                    decoration: BoxDecoration(
                      color: i == selectedIndex
                          ? CleoColors.primaryCocoa.withValues(alpha: 0.1)
                          : Colors.transparent,
...
class CleoScaffold extends StatelessWidget {
  const CleoScaffold({
    super.key,
    required this.body,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: backgroundColor ??
          (isDark ? CleoColors.deepCocoa : CleoColors.bgMain),
      appBar: appBar,
      body: body,
    );
  }
...
class CleoTheme {
  ...
  /// Light theme with Cleo design tokens.
  /// Use alongside existing SocelleTheme — apply CleoTheme to new
  /// intelligence surfaces, keep SocelleTheme for gap-recovery surfaces.
  static ThemeData get light {
    return ThemeData(
      useMaterial3: true,
      scaffoldBackgroundColor: CleoColors.bgMain,
      fontFamily: 'Inter',
      splashFactory: NoSplash.splashFactory, // Cleo uses no ripple
      highlightColor: Colors.transparent,

      colorScheme: ColorScheme.fromSeed(
        seedColor: CleoColors.primaryCocoa,
        primary: CleoColors.primaryCocoa,
        onPrimary: CleoColors.bgMain,
        secondary: CleoColors.neutralBeige,
        surface: CleoColors.bgMain,
        onSurface: CleoColors.primaryCocoa,
      ),
      ...
      textTheme: const TextTheme(
        displayLarge: CleoText.displayLarge,
        displayMedium: CleoText.displayMedium,
        headlineLarge: CleoText.headlineLarge,
        headlineMedium: CleoText.headlineMedium,
        bodyLarge: CleoText.bodyLarge,
        bodyMedium: CleoText.bodyMedium,
        labelMedium: CleoText.labelUppercase,
        labelLarge: CleoText.buttonLabel,
      ),
    );
  }
}
